...
export function AdminStudio({ userName, userEmail }: { userName: string; userEmail: string }) {
  ...
  const [deleteConfirmTarget, setDeleteConfirmTarget] = useState<{ clientKey: string, slug?: string } | null>(null);
  ...
  function deleteTargetDraft(clientKey: string, slug?: string) {
    setDeleteConfirmTarget({ clientKey, slug });
  }

  function executeDelete() {
    if (!deleteConfirmTarget) return;
    const { clientKey, slug } = deleteConfirmTarget;
  ...
  useEffect(() => {
    const handlePointerMove = () => setCommandbarVisible(true);
      ...
      <div className={`studio-workspace ${isLeftVisible ? "" : "left-collapsed"} ${isRightVisible ? "" : "right-collapsed"} ${isResizingActive ? "is-resizing" : ""}`}>
        ...
        <div className={`floating-commandbar ${commandbarVisible ? "" : "hidden"}`}>
          ...
          <div className="workspace-view-controls" role="group" aria-label="편집 화면 패널 보기">
            <button type="button" className="has-tip-top focus-mode-control" data-help={focusMode ? "클릭하면 미리보기와 목록 패널을 엽니다." : "클릭하면 양쪽 패널을 닫고 편집에 집중합니다."} aria-label={focusMode ? "집중 모드 종료" : "집중 모드 시작"} aria-pressed={focusMode} onClick={toggleFocusMode}><FocusIcon size={17}/><span>{focusMode ? "미리보기 켜기" : "미리보기 중.."}</span></button>
          </div>
          ...
          <div className="studio-actions"><button type="button" className="has-tip-top" data-help="자동 저장은 현재 기기(브라우저)에만 임시 보관됩니다.&#10;다른 PC나 기기에서도 이어서 작업하려면 [저장]을 눌러 서버에 동기화해 주세요." onClick={() => void save("save")} disabled={uploading || savePhase === "saving"}>{draft.isPublished ? "저장 및 반영" : "저장"}</button><button className="has-tip-top publish-button" data-help="작성한 내용이 누구나 볼 수 있는 정식 게시글로 업로드됩니다.&#10;언제든 다시 비공개로 돌릴 수 있습니다." type="button" onClick={() => void save("publish")} disabled={uploading || savePhase === "saving"}>{draft.isPublished ? "공개됨" : "공개하기"} <ArrowIcon size={17}/></button></div>
        </div>
      </div>
