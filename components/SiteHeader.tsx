    ...
    <header className="site-header">
      <div className="header-inner page-shell">
        <Link href="/" className="brand" aria-label="구세군 마포영문 기획서 아카이브 홈">
          ...
          <span className="brand-type"><b>구세군 마포영문</b><span>아카이브</span></span>
        </Link>
        <nav className="desktop-nav" aria-label="주요 메뉴">
          {links.map((link) => <Link key={link.href} href={link.href} className={pathname === link.href ? "active" : ""}>{link.label}</Link>)}
          <Link href="/admin" className="write-entry"><PlusIcon size={16}/>새 글 쓰기</Link>
        </nav>
      ...
      <nav id="mobile-menu" className={`mobile-menu ${open ? "open" : ""}`} aria-label="모바일 메뉴">
        ...
        <Link onClick={() => setOpen(false)} href="/admin" className="write-entry"><PlusIcon size={17}/>새 글 쓰기</Link>
      </nav>
    </header>
    <Link href="/admin" className="mobile-write-entry" aria-label="새 글 쓰기"><PlusIcon size={18}/><span>새 글 쓰기</span></Link>
  </>;
}
